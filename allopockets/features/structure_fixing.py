...
def get_fixed_atoms(f, pdb, orig_atoms, no_alt=True, no_hetatm=True, standard_resnames=True):
    ...
    """
    if PDBFixer is None or unit is None:
        raise ImportError(
            "pdbfixer and openmm/simtk are required for get_fixed_atoms. Please install pdbfixer and openmm."
        )
    ...
    newcols = {
        c: (f"{c}_new" if c in orig_atoms else c) for c in newdf.columns if c not in mergecols
    }
    ...
    merged.loc[:, ["Cartn_x", "Cartn_y", "Cartn_z"]] = merged.loc[
        :, ["Cartn_x_new", "Cartn_y_new", "Cartn_z_new"]
    ].values
    ...
    if standard_resnames:
        d = {**inverse_aa_codes, **IUPACData.protein_letters_3to1_extended}
        try:
        ...
        for c in ["label_comp_id", "auth_comp_id"]:
            special = (
                {
                    k: seq1(v)
                    for k, v in dict(
                        pd.DataFrame(orig_map).loc[:, [c, "parent_comp_id"]].values.tolist()
                    ).items()
                }
                if orig_map
                else {}
            ...
            merged.loc[:, c] = [
                seq3(seq1(comp, custom_map={**d, **special})).upper() for comp in merged[c]
            ]
...
def get_fixed_structure(pdb, p, minimal_chains, path, save=False):
    ...
    with tempfile.NamedTemporaryFile("w+", suffix=".cif") as tempf:
        ...
        tempwriter.write({pdb.entry_id.upper(): {"_atom_site": orig_atoms.to_dict("list")}})
