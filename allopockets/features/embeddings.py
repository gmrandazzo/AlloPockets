...
def get_embs(sequence):
    ...
    for i, chunk in enumerate(input_chunks):
        ...
        chunk_attention = torch.cat(
            [chunk_attention, torch.tensor([0] * (max_length - len(chunk_attention)))]
        )
        ...
        chunk_embeddings = outputs.last_hidden_state.squeeze(0)[no_padding][
            1:-1, :
        ]  # Exclude padding, CLS (first) and EOS (last)
