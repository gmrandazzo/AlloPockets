...
featsd = {
    ...
    "FreeSASAF": {
        "freesasa": [
            ...
            "relative-area_total",
            "relative-area_polar",
            "relative-area_apolar",
            "relative-area_main-chain",
        ]
    },  # 'relative-area_side-chain' is not used as it's always NaN for glycines
    "DSSPF": {
        "dssp": [
            "auth_asym_id",
            "auth_seq_id",
    ...
    "TransferEntropyF": {
        "transfer_entropy": ["auth_asym_id", "auth_seq_id", "pdbx_PDB_ins_code", "TE"]
    },
    "HHBlitsF": {
        "hhblits": [
            "label_asym_id",
...
def get_labels(residues, sites):
    ...
    labels.iloc[
        residues.merge(sites_res, how="left", indicator=True).query("_merge == 'both'").index
    ] = 1
...
def get_pdb_features(cif, sites, features_path):
    ...
    assert all(
        res.lower().capitalize() in IUPACData.protein_letters_3to1
        for res in cif.residues["label_comp_id"].unique()
    ), (
        pdb,
        "Unrecognized amino acid 3-letter name:",
        set(res.lower().capitalize() for res in cif.residues["label_comp_id"].unique())
        - set(IUPACData.protein_letters_3to1),
    )
    ...
    features = pd.concat(
        (
            ...
            labels,
            encode_column(
                cif.residues["label_comp_id"].apply(
    ...
    for fcname in featsd.keys():
        ...
        for feat in pd.read_pickle(f"{features_path}/features/{pdb}/{fcname}.pkl"):
            ...
            if len(result.columns) != len(featsd[fcname][fname]):
                print(
                    (pdb, fcname, fname),
                    "Not all feature columns",
                    set(featsd[fcname][fname]) - set(result.columns),
                )
            if len(result) != len(residues):
                print(
                    (pdb, fcname, fname),
                    "Different # of residues:",
                    len(result),
                    f"(original {len(residues)})",
                )
            ...
            if fname == "dssp":
                result = pd.concat(
                    (
                        ...
                        encode_column(
                            result["secondary structure"], ["H", "B", "E", "G", "I", "T", "S", "-"]
                        ),
                    ),
                    axis=1,
                )
            # For HHBlits, keep only the minimal residue ID columns and transform feature columns to float
                ...
                result = pd.concat(
                    (
                        ...
                        result.drop(columns=[c for c in result.columns if c in residues]).astype(
                            float
                        ),
                    ),
                    axis=1,
            ...
            if (
                not result[[c for c in result.columns if c not in residues.columns]]
                .dtypes.apply(lambda c: c.kind in "if")
                .all()
            ):
                print(
                    (pdb, fcname, fname),
                    "Not all columns are numeric:",
                    result[[c for c in merged.columns if c not in residues.columns]].dtypes,
                )
            if (
                result[[c for c in result.columns if c not in residues.columns]]
                .isna()
                .all(axis=1)
                .any()
            ):
            ...
            if len(merged) != len(residues):
                print(
                    (pdb, fcname, fname),
                    "Different # of residues after merging feature:",
                    len(merged),
                    f"(original {len(residues)})",
                )
            ...
            (
                merged.set_axis(
                    pd.MultiIndex.from_tuples(
                        [
                            ("Residues" if c in residues.columns else fcname[:-1], c)
        ...
        if len(result) != len(residues):
            print(
                (pdb, fcname, fname),
                "Different # of residues after merging feature class:",
                len(features),
                f"(original {len(residues)})",
            )
